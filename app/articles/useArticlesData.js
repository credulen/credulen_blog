...
export const useArticle = () => {
  ...
  const paginatedArticles = useQuery({
    ...
    queryFn: () =>
      axios.get(
        `https://strapi-blcj.onrender.com/api/articles?sort=publishedAt:desc&${queryStr}&pagination[page]=${page}&pagination[pageSize]=6`
      ),
  ...
  const allArticles = useQuery({
    queryKey: ["allArticle"],
    queryFn: () =>
      axios.get(`https://strapi-blcj.onrender.com/api/articles?${queryStr}`),
    refetchOnMount: false,
    refetchOnWindowFocus: false,
    refetchOnReconnect: false,
    initialData: undefined,
...
const arrayFromPaginatedArticles = (paginatedArticles) => {
  ...
  for (
    let i = 1;
    i <= paginatedArticles.data.data.meta.pagination.pageCount;
    i++
  ) {
