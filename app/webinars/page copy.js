...
const Webinars = () => {
  ...
  const { getAllWebinarData } = useWebinarData();
  ...
  if (getAllWebinarData.isLoading) {
    return <IsLoading />;
  }
  ...
  return (
    <div className="container">
      ...
      <div className="mt-5 text-dark">
        ...
        <div className="row g-4">
          {getAllWebinarData?.data?.data?.data?.map((webinar) => {
            ...
            if (
              getAllWebinarData.data?.data?.data?.attributes
                ?.is_upcoming_webinar === true
            ) {
              return (
